  ...
  async execute(organizationId: string, saleId: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const item of sale.saleItems) {
        ...
        const deductedLots = itemLots.filter((lot: any) => lot.isStockDeducted);

        if (deductedLots.length > 0) {
          for (const saleItemLot of deductedLots) {
            ...
            await tx.stockMovement.create({
              data: {
                ...
                type: 'SALE_REVERTED',
                sourceDocument: `Estorno Venda #${sale.orderNumber}`,
              },
            });
          }
        } else {
