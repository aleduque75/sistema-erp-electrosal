...
export function PayAccountForm({ account, onSave }: PayAccountFormProps) {
  ...
  const [contaContabilOptions, setContaContabilOptions] = useState<{ value: string; label: string }[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
    ...
    ]).then(([contasCorrentesRes, pureMetalLotsRes, contasContabeisRes]) => {
      ...
      setContaContabilOptions(contasContabeisRes.data.map((cc: any) => ({ value: cc.id, label: `${cc.codigo} - ${cc.nome}` })));
    }).catch(() => {
