...
export function SaleDetailsView({ sale, onReceivePayment }: SaleDetailsViewProps) {
  ...
  const allReceivedTransactions = sale.accountsRec
    .filter(ar => ar.received || ar.transacoes.length > 0)
    .flatMap(ar => ar.transacoes.map(t => ({ ...t, receivedAt: ar.receivedAt })));
              ...
              <TableBody>
                {allReceivedTransactions.map((transaction) => (
                  <TableRow key={transaction.id}>
                    ...
                    <TableCell>{transaction.contaCorrente?.nome || (transaction.goldAmount ? 'Crédito de Metal' : 'N/A')}</TableCell>
